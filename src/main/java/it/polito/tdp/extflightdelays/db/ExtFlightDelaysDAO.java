...
//		METODO PER PRENDERE TUTTI I VERTICI DEL GRAFO
		String sql = "SELECT tmp.ID, tmp.IATA_CODE, COUNT(*) AS N "
				...
				+ "HAVING N >= ?";
		List<Airport> vertici = new LinkedList<Airport>();
	...
	public List<Rotta> getRotte(Map<Integer, Airport> idMap) {
		...
		String sql = "SELECT f.ORIGIN_AIRPORT_ID, f.DESTINATION_AIRPORT_ID, COUNT(*) AS nVoli "
				+ "FROM flights f "
		...
		try {
			...
			while (rs.next()) {
				rotte.add(new Rotta(idMap.get(rs.getInt("ORIGIN_AIRPORT_ID")), idMap.get(rs.getInt("DESTINATION_AIRPORT_ID")), rs.getInt("nVoli")));
			}
